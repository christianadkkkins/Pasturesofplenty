...
This repository contains a cross-domain geometric early-warning study spanning LTST cardiac analysis, LTST-derived ESP32 contract validation, geomagnetic storm precursor detection on OMNI HRO2 one-minute solar wind data, and distribution-grid event analysis on micro-PMU data. The main detector-style results live in [RESULTS_SUMMARY.csv](RESULTS_SUMMARY.csv): the finalized cardiac transition-HMM is visible before `7524/8000` ST episodes (`94.1%`) with median lead `429` beats and lead-aware specificity `0.671`, while the finalized layered solar HMM moves before `546/589` storms with median lead `279.5` minutes and minute-level specificity `0.756`. The embedded contract result is summarized separately in [../RESULTS_ESP32.md](../RESULTS_ESP32.md).
...
- `reproduce.py`: cross-platform Python runner for the cardiac cohort build, finalized cardiac HMM, finalized ESP32 contract simulation, finalized solar HMM, and Figure 2 solar plot reproduction.
- `reproduce_all.ps1`: runs the cardiac base cohort build, the cardiac transition-HMM, and the solar layered HMM in sequence.
- `reproduce_esp32_contract.ps1`: reruns the LTST-derived ESP32 fixed-point contract simulation from a completed LTST run directory.
...
- `../ESP32/ltst_esp32_contract_sim.py`: host-side fixed-point emulation of the frozen LTST V1 packet.
- `../ESP32/ltst_v1_export_replay_samples.py`: regenerates replay assets for the ESP32 sample-replay path.
...
The solar pipeline entrypoint, [solar.py](../solar.py), is included at repository root. The cardiac HMM entrypoint is [artifact/ltst_transition_hmm.py](../artifact/ltst_transition_hmm.py). The ESP32 contract entrypoint is [../ESP32/ltst_esp32_contract_sim.py](../ESP32/ltst_esp32_contract_sim.py).

## Final Frozen Runs Summarized Here

- Cardiac HMM: `artifact/runs/ltst_full_86_20260405T035505Z/transition_hmm_onset_cooldown32`
- ESP32 contract simulation: `artifact/runs/ltst_full_86_20260405T035505Z/esp32_contract_sim`
...
4. Run `python results/reproduce.py esp32-contract --run-dir artifact/runs/<your_ltst_run>` to rerun the LTST-derived ESP32 contract simulation.
5. Run `python results/reproduce.py solar-hmm` to regenerate the finalized layered solar outputs.
...
- The ESP32 replay assets and contract simulation are also second-stage analyses. They consume a completed LTST cohort run rather than raw PhysioNet records directly.
